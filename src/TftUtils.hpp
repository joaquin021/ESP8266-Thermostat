...
void updateTargetTemp()
{
  ...
  dtostrf(thermostatData.targetTemp, 4, 1, targetTempAux);
...
void updateCircleColor(float temperature)
{
  //draw big circle
  unsigned char i;
  if (thermostatData.action.equals("heating"))
  {
    for (i = 0; i < 10; i++)
      tft.drawCircle(120, 120, 80 + i, ILI9341_RED);
  }
  else if (thermostatData.action.equals("idle"))
  {
    for (i = 0; i < 10; i++)
...
void changeThermostatMode(const char *mode, size_t maxlen)
{
  strncpy(thermostatData.mode, mode, maxlen - 1);
  thermostatData.mode[maxlen - 1] = '\0';
}
...
void detectButtons(int x, int y)
{
  ...
  if (x >= 200)
  {
    if (y <= 40)
    {
      thermostatData.targetTemp = thermostatData.targetTemp + 0.5;
      updateTargetTemp();
    }
    else if (y >= 200 && y <= 240)
    {
